...
char **gen_empty_map(char **info, char **pixmap){
	...
	sprintf(ptr," c None");
	map[1] = ptr;
	ptr = *pixmap = calloc(1,xmax * zoom);
	for(k=0; k < zoom * xmax ; k++)
		(*pixmap)[k] = ' ';
...
int main(int argc, char **argv){
	...
	while ((i = getopt(argc, argv, "X:Y:d:f:s:o:x:y:zh")) != EOF){
		switch (i){
			case 'd':
	...
	if (directory == NULL){
		for(i=optind ; i < argc  && exit_flag != 1 ; i++){
			if((argv[i][strlen(argv[i]) - 1] == '1' ||
			    argv[i][strlen(argv[i]) - 1] == '2' ||
			    argv[i][strlen(argv[i]) - 1] == '3') &&
			   (argv[i][strlen(argv[i]) - 2] == 'k' ||
			    argv[i][strlen(argv[i]) - 2] == 'K') &&
			   (argv[i][strlen(argv[i]) - 3] == 'b' ||
			    argv[i][strlen(argv[i]) - 3] == 'B') 
					){
				main_loop(argv[i]);
				sleep(secs);
